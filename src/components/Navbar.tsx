 ...
 * Gestiona la navegación principal de la SPA, el menú adaptativo (hamburguesa) para móviles
 * y el interruptor global (switch) para alternar entre el Modo Claro y el Modo Oscuro.
 */
const Navbar = () => {
    ...
    // Controla el tema visual (por defecto inicia en oscuro)
    const [isDarkMode, setIsDarkMode] = useState(true);
    
    // Controla la visibilidad del menú desplegable en resoluciones móviles
    ...
    /**
     * Sincroniza el estado local de 'isDarkMode' con la clase 'dark' en el elemento raíz HTML.
     * Esta es la práctica estándar en Tailwind CSS para habilitar sus utilidades dark:*.
     */
    useEffect(() => {
        if (isDarkMode) {
    ...
    // Array de enlaces para renderizado dinámico, facilitando la adición de nuevas secciones.
    const navLinks = [
        ...
        <nav className="fixed w-full top-0 z-50 bg-primary/80 backdrop-blur-md border-b border-secondary transition-colors duration-300">
            <div className="flex items-center justify-between h-20 px-8 md:px-20 lg:px-32">
                ...
                <div className="flex items-center gap-4 md:gap-6">
                    ...
                    {/* MENÚ DESKTOP (Visible solo en pantallas md en adelante) */}
                    <div className="hidden md:flex items-center gap-6">
                        ...
                        {/* Separador visual entre los links y el switch de tema */}
                        <div className="w-px h-6 bg-textDim/30"></div>
                    </div>

                    {/* INTERRUPTOR DE TEMA (Dark/Light Mode Switch) */}
                    <button
                        type="button"
                    ...
                    >
                        ...
                        {/* Ícono de Sol (Aparece a la izquierda) */}
                        ...
                        {/* Ícono de Luna (Aparece a la derecha) */}
                    ...
                    >
                        {/* Renderizado condicional del ícono (X para cerrar, líneas para abrir) */}
                        {isMenuOpen ? (
            ...
            {/* Se utiliza max-height y opacidad para animar la apertura/cierre sin desmontar el componente */}
            <div
                ...
                <div className="flex flex-col items-center gap-6 py-6">
                    {navLinks.map((link) => (
                        <a
                            key={link.name}
                            href={link.href}
                            // Al tocar un enlace, el menú móvil se cierra automáticamente
